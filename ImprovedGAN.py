...
class ImprovedGAN(object):
    ...
    def trainD(self, x_label, y, x_unlabel):
        ...
        prob_label = torch.gather(output_label, 1, y)  # log e^x_label = x_label
        ...
        acc = torch.mean((output_label.max(-1)[1].unsqueeze(dim=1) == y).float())
    ...
    def train(self):
        ...
        tile_labeled = TensorDataset(t1.repeat(times, 1), t2.repeat(times, 1))
        gn = 0
        for epoch in range(self.args.epochs):
            ...
            for (unlabel1, _label1) in unlabel_loader1:
                ...
                # embedding
                x = preprocessor.embed(x)
                unlabel1 = preprocessor.embed(unlabel1)
                unlabel2 = preprocessor.embed(unlabel2)
                # end embedding
                if args.cuda:
                    x, y, unlabel1, unlabel2 = x.cuda(), y.cuda(), unlabel1.cuda(), unlabel2.cuda()
                ll, lu, acc = self.trainD(x, y, unlabel1)
                # if epoch > 1 and (ll > 1 or lu > 1):
                #     ll, lu, acc = self.trainD(x, y, unlabel1)
                ...
                if (batch_num + 1) % self.args.log_interval == 0:
                    ...
                    with torch.no_grad():
                        ...
                        self.writer.add_histogram('fake_feature',
                                                  self.D(self.G(self.args.batch_size,
                                                         cuda=self.args.cuda).view(x.size()),
    ...
    def predict(self, x):
        with torch.no_grad():
            output = self.D(Variable(x), cuda=self.args.cuda)
            ret = torch.max(output, 1)[1].data
        return ret

    def eval(self):
        self.G.eval()
        self.D.eval()
        test_loader = DataLoader(self.test, batch_size=1, shuffle=True, num_workers=4)
        tot_corr = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--d_repeat', type=int, default=5, metavar='DR',
    ...
    gan = ImprovedGAN(Generator(100), Discriminator(),
                      preprocessor.labeled_dataset(), preprocessor.unlabeled_dataset(), preprocessor.test_dataset(),
                      args)
    gan.train()
    gan.eval()
